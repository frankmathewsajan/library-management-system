...
class Library:
    ...
    def set_image(uid: str, data: bytes) -> bool:
        DB.update("library", {"image_data": data}, f"uid = '{uid}'")
        return True
    ...
    def set_books(data: dict, search: str) -> bool:
        ...
        for book in data.get("items", []):
            try:
                ...
                author = (
                    encode(volume_info.get("authors", [None])[0])
                ...
                if author and pgno != 0:
                    ...
                    saleability, price = (
                        (False, 0)
                        if sales_info.get("saleability") == "NOT_FOR_SALE"
                        else (True, sales_info.get("listPrice", {}).get("amount", 0))
                    )
                    ...
                    book_data = {
                        ...
                        "image_data": None,
                        "author": author,
                        "publisher": encode(volume_info.get("publisher", ""))
                        if volume_info.get("publisher")
                        else None,
                        "description": encode(volume_info.get("description", ""))
        ...
        try:
            DB.insert("searches", {"search": search, "username": Accounts.username()})
