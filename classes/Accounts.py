...
import bcrypt
import re
from classes.Constants import CHECKOUT_LIMIT, DUE_DAYS, FINE, PASSWORD_CONDITIONS, PASSWORD_REGEX, USERNAME_CONDITIONS, USERNAME_REGEX
...
class Accounts:
    ...
    def searches() -> list:
        ...
        tuple_list = DB.get(
            ["username", "search"], "searches", f"username = '{username}' or username = 'SYSTEM'"
        )
    ...
    def register(username: str, password: str, window) -> bool:
        ...
        if (user_exist and valid_input):
            ...
            DB.insert(
                "users", {"username": username, "password": hashed_password, "auth": True}
            )
            DB.insert('config',{
            'id':username,
            'value':False
    ...
    def checkout_book(uid):
        ...
        ch = sg.popup_ok_cancel(f"Confirm Book Check Out\n\nBook Name : {book_name[0]} by {decode(book_name[1])}",  title="Confirm Check-Out")
        ...
        if uid and len(borrowed) < CHECKOUT_LIMIT and uid not in borrowed:
            DB.insert("books", {"uid": uid, "type": "BORROWED", "username": username,"date": datetime.now()})
        ...
        sg.Popup(f"Name : {book_name[0]} by {decode(book_name[1])}\n\n{msg}", title=f'Check-Out | {title}', keep_on_top=True)
    ...
    def add_money(amount: int) -> int:
        ...
        if Accounts.block(2) and balance > 0: 
            Accounts.block(1) 
            sg.popup_ok('Your bebt is paid, so all transactions has been unblocked', title='Transactions Unblocked')  
        return balance
    ...
    def return_book(uid):
        ...
        ch = sg.popup_ok_cancel(f"Confirm Book Check In\nBook Name : {book_name[0]} by {decode(book_name[1])}\nDue Date : {due}\nFine : ₹{fine}",  title="Confirm Check-In")
        if (True if ch=="OK" else False):
            ...
            count = (int(DB.get(["checked_out"], "users", f"username = '{username}'")[0][0]) - 1)
            DB.update("users", {"checked_out": count}, f"username = '{username}'")
